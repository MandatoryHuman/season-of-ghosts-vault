 ...
 */
export function installNativeDeps(nativeDeps, options) {
    ...
    for (const [pkg, pluginRanges] of merged) {
        ...
        if (options.verbose) {
            const sources = [...pluginRanges.entries()]
                ...
                .join(", ");
            console.log(styleText("cyan", `→`), `Native dep ${styleText("bold", pkg)} required by: ${sources}`);
        }
        if (uniqueRanges.length === 1) {
            ...
            if (options.verbose) {
                console.warn(styleText("yellow", `⚠`), `Multiple version ranges for ${pkg}: ${uniqueRanges.join(", ")}. npm will attempt to resolve a compatible version.`);
            }
    ...
    if (installArgs.length === 0)
        return;
    if (options.verbose) {
        console.log(styleText("cyan", `→`), `Installing ${installArgs.length} native package(s) into Quartz root...`);
    }
    try {
        ...
        if (options.verbose) {
            console.log(styleText("green", `✓`), `Installed native dependencies: ${[...merged.keys()].join(", ")}`);
        }
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        console.error(styleText("red", `✗`), `Failed to install native dependencies. This may indicate incompatible version ranges across plugins.\n` +
...
export async function installPlugin(spec, options = {}) {
    ...
    if (options.verbose) {
        ...
        console.log(styleText("cyan", `→`), `Cloning ${spec.name} from ${spec.repo}${refSuffix}${subdirSuffix}...`);
    }
    if (spec.subdir) {
 ...
 */
export async function installPlugins(sources, options = {}) {
 ...
 */
export function getPluginEntryPoint(name) {
 ...
 */
export function getPluginSubpathEntry(name, subpath) {
    const pluginDir = getPluginDir(name);
    const searchDir = pluginDir;
 ...
 */
export async function updatePlugins(options = {}) {
    if (!fs.existsSync(PLUGINS_CACHE_DIR)) {
...
export function validatePluginExternals(pluginName, entryPoint, _options) {
    try {
...
export async function regeneratePluginIndex(options = {}) {
    if (!fs.existsSync(PLUGINS_CACHE_DIR)) {
    ...
    for (const [pluginName, { overridable }] of pluginExports) {
        ...
        if (conflicting.length > 0 && options.verbose) {
            for (const n of conflicting) {
                console.warn(styleText("yellow", `⚠`), `Export "${n}" conflicts across plugins — use plugins["${pluginName}"].${n} in quartz.ts`);
            }
        }
    }
    lines.push("");
    ...
    if (options.verbose) {
        console.log(styleText("green", `✓`), `Regenerated plugin index with ${pluginDirs.length} plugins`);
    }
}
