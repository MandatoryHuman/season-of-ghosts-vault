...
import { parsePluginSource, installPlugin, installNativeDeps, getPluginEntryPoint, toFileUrl, isLocalSource, } from "./gitLoader";
...
function resolveConfigPath() {
    ...
    if (fs.existsSync(LEGACY_PLUGINS_JSON_PATH))
...
export async function loadQuartzConfig(configOverrides) {
    ...
    if (footerEntry) {
        ...
        if (footerReg) {
            if (typeof footerReg.component === "function" && !("displayName" in footerReg.component)) {
                ...
                footer = componentRegistry.instantiate(footerReg.component, Object.keys(opts).length > 0 ? opts : undefined);
            }
            else {
...
function buildLayoutForEntries(entries, layoutConfig) {
    ...
    for (const entry of entries) {
        ...
        if (typeof reg.component === "function" && !("displayName" in reg.component)) {
            ...
            component = componentRegistry.instantiate(reg.component, optsArg);
        }
        else {
