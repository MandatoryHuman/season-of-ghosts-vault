...
function makeContent(entries) {
  return entries.map((e) => {
    const vfile = {
      data: {
        slug: e.slug,
        relativePath: e.relativePath ?? `${e.slug}.md`,
        filePath: e.filePath ?? `/vault/${e.relativePath ?? `${e.slug}.md`}`,
      },
    }
...
describe("detectSlugCollisions", () => {
  ...
  test("ignores entries without a slug", () => {
    const content = [
      ...
      [{ type: "root", children: [] }, { data: { slug: undefined, relativePath: "broken.md" } }],
    ]
    assert.deepStrictEqual(detectSlugCollisions(content), [])
  })
