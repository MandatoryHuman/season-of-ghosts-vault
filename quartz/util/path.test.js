...
describe("transforms", () => {
  ...
  test("slugifyFilePath + simplifySlug end-to-end canonicalization", () => {
    ...
    const indexStyle = path.simplifySlug(path.slugifyFilePath("characters/index.md"))
    const folderNameStyle = path.simplifySlug(path.slugifyFilePath("characters/characters.md"))
...
describe("link strategies", () => {
  const allSlugs = ["a/b/c", "a/b/d", "a/b/index", "e/f", "e/g/h", "index", "a/test.png"]
  describe("absolute", () => {
    const opts = {
      strategy: "absolute",
      allSlugs,
...
describe("resolveRelative", () => {
  test("from index", () => {
    assert.strictEqual(path.resolveRelative("index", "index"), "./")
    assert.strictEqual(path.resolveRelative("index", "abc"), "./abc")
    assert.strictEqual(path.resolveRelative("index", "abc/def"), "./abc/def")
    assert.strictEqual(path.resolveRelative("index", "abc/def/ghi"), "./abc/def/ghi")
  })
  test("from nested page", () => {
    assert.strictEqual(path.resolveRelative("abc/def", "index"), "../")
    assert.strictEqual(path.resolveRelative("abc/def", "abc"), "../abc")
    assert.strictEqual(path.resolveRelative("abc/def", "abc/def"), "../abc/def")
    assert.strictEqual(path.resolveRelative("abc/def", "ghi/jkl"), "../ghi/jkl")
  })
  test("with index paths", () => {
    assert.strictEqual(path.resolveRelative("abc/index", "index"), "../")
    assert.strictEqual(path.resolveRelative("abc/def/index", "index"), "../../")
    assert.strictEqual(path.resolveRelative("index", "abc/index"), "./abc/")
    assert.strictEqual(path.resolveRelative("abc/def", "abc/index"), "../abc/")
  })
  test("with simple slugs", () => {
    assert.strictEqual(path.resolveRelative("abc/def", ""), "../")
    assert.strictEqual(path.resolveRelative("abc/def", "ghi"), "../ghi")
    assert.strictEqual(path.resolveRelative("abc/def", "ghi/"), "../ghi/")
  })
})
